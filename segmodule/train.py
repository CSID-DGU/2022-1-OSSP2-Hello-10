...
class Trainer(object):
    def __init__(self,):
        ...
        model = DeepLabv3_plus(nInputChannels=3, n_classes=self.nclass,
                               os=16, pretrained=settings.pretrained, _print=True)
        ...
        if settings.use_balanced_weights:
            ...
            if os.path.isfile(classes_weights_path):
                ...
                weight = calculate_weigths_labels(
                    settings.dataset, self.train_loader, self.nclass)
        ...
        if settings.cuda:
            self.model = torch.nn.DataParallel(
                self.model, device_ids=settings.gpu_ids)
        ...
        if settings.resume:
            if not os.path.isfile(settings.checkpoint):
                raise RuntimeError(
                    "=> no checkpoint found at '{}'" .format(settings.checkpoint))
        ...
        for i, sample in enumerate(tbar):
            ...
            if i % (num_img_tr // 10) == 0:
                ...
                self.summary.visualize_image(
                    self.writer, settings.dataset, image, target, output, global_step)

        self.writer.add_scalar('train/total_loss_epoch', train_loss, epoch)
        print('[Epoch: %d, numImages: %5d]' %
              (epoch, i * settings.batch_size + image.data.shape[0]))
    ...
    def validation(self, epoch):
        ...
        if new_pred > self.best_pred:
            ...
            self.saver.save_checkpoint({
                'epoch': epoch + 1,
                'state_dict': self.model.module.state_dict() if settings.cuda else self.model.state_dict(),
                'optimizer': self.optimizer.state_dict(),
                'best_pred': self.best_pred,
            }, is_best)
